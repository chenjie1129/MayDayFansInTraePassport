...
- 将 `Answer` 接入 DeepSeek 官方 Chat Completions API：Wi-Fi 配网页面新增可选的遮罩 API Key 输入，Key 与已验证的 Wi-Fi 配置都只保留在本次开机的 RAM 中；`Answer` 复用持续在线的网络管理器，每次抽取都调用 DeepSeek，并明确展示未配网、缺少 Key、鉴权失败、限流、超时等状态。由于设备只内置固定中文子集字库，且当前玩法没有文字问题输入，模型被严格限制为返回 `0..15` 的 JSON 答案编号，页面映射到本地答案，拒绝直接渲染不可控模型文本。配网热点密码同时升级为每次进入页面随机生成，并仅显示在设备屏幕上。
- 将菜单中的 `Wi-Fi` 从“仅扫描热点”升级为完整联网应用：通行证会创建 WPA2 临时热点和网页配网入口，供手机选择 2.4 GHz 网络；凭证仅保存在 RAM，设备屏幕同步显示配网、连接、IP、RSSI 与公网检查状态，信号中断后自动重连。连接成功后，退出页面不再中断网络；再次进入 `Wi-Fi` 会直接显示当前 SSID、IP、RSSI 与公网状态。`Places` 会在自己的 Wi-Fi/BLE 分时周期前暂停共享连接，退出后自动恢复。修复提交配网表单时的 HTTP 任务栈溢出：URL 解码缓冲改为可主动清零的堆内存，并提高 HTTP 任务保护栈。Host 契约测试覆盖凭证处理、仅 POST 提交、持续联网所有权、资源释放顺序及 `Places` 无线资源仲裁。
- 参考 `chenjie1129/MayDayFansInCodex` 的圆形橙色卜卜形象，统一更新萝卜圈子图标和六阶段宠物：三片软叶、蓝白大眼与黑色波浪嘴在 32×32、48×48 下均保持可辨识。参考帧及其个人／非商业使用说明保存在 `main/assets/source`；同步重生成 RGB565A8 资源、深浅色预览，并将配饰遮脸检查改为按蓝色眼睛定位脸部。
- 将 `Places` 扩展为 PRD1 离线社交通行证体验：每 60 秒按“限时 Wi-Fi 地点指纹扫描 → 1 秒射频空档 → BLE 广播／Observer 交替窗口 → 自动 light sleep”分时运行，Wi-Fi 与 NimBLE 在各自阶段后完整 deinit，并为每个阶段记录内部 RAM 最小空闲 heap 与最大连续块变化。新地点提供家／工位／咖啡／地铁／健身房／自定义六种预设命名，持久化图标、到访次数和基于单调时钟的累计停留时长，并新增按时长排序的档案和“本次开机轨迹”。8 字节厂商自定义 BLE 协议包含随机持久化匿名 ID、5 种信号码、由 top-3 BSSID 排序派生的地点哈希、相互解耦的圈子码／图标，以及只增不减的六阶段卜卜形态；隐身模式只扫描不广播，熟客识别默认关闭、用户主动开启后最多保存 32 个 LRU 档案，并展示最常相遇的本机地点。同圈设备走相遇或可配置的九号球安静反馈，不同圈子只增加角落圆点；聚集模式使用固定 128 字节 bitmap＋8 个图标计数器，暂停熟客写入，并保存最近 8 条纪念记录。已接入可替换表形式的五月天圈子图标和通用蔬菜宠物素材；进化只由地点数驱动且不回退，性格按显式优先级实时派生，呼吸／发呆／碰面动画只用 LVGL 位移。两个打包 NVS blob 原始数据合计 2,284 字节；主机测试覆盖三版载荷、广播过滤、地点哈希顺序不变与碰撞抽样、熟客 LRU、2,000 ID 固定内存聚集计数、纪念淘汰、进化边界、性格优先级和无损发呆态。
- 修复 `Places` 玩法「设备没动却被识别成新地点」的误判：真机日志显示同一地点 Jaccard 得分在 600‰~1000‰ 间抖动——top-8 指纹有 1~2 个边界 BSSID 因 RSSI 抖动进出，单次扫描跌破 50% 阈值就立即建了重复地点。现指纹改为保留信号最强的 12 个 BSSID；相似度改用 Overlap 系数（交集 / min(|A|,|B|)），本次扫描多听到的弱 AP 不再拉低得分；新增迟滞状态机（`place_fp_observe`，`main/place_fp.c` 纯逻辑）：350‰~499‰ 为灰区，绝不建点并在 4 秒后快速重扫，低于 350‰ 也必须连续 2 次扫描确认才提交新地点（空库首个地点仍直接建立）；确认期间页面显示 `CHECKING...` / `WEAK n% - RESCAN SOON`。NVS blob 版本升至 2（1,416 字节；旧版 8-BSSID 的 v1 blob 会被识别并安全重置，绝不按新布局误解析），主机测试新增 Overlap 计算、阈值边界、V0.1 误判回归（5/8 共享：Jaccard 454‰ vs Overlap 625‰）与全部迟滞状态转换用例。
- 新增 `Places` 玩法（「场所记忆」）：不联网、不用 GPS 的离线地点识别。每 60 秒（或 OK 短按立即触发）做一次一次性主动 Wi-Fi 扫描，取信号最强的 8 个 BSSID 作为当前地点指纹；每次扫描后由 `components/bsp` 的 `bsp_wifi_scan_once()` 完整 stop + deinit 释放协议栈（`WIFI_STORAGE_RAM` 不写 NVS，默认 STA netif 随扫描创建/销毁）。指纹与已保存地点用纯整数 Jaccard 相似度比对，重叠 ≥50%（含边界）判为同一地点；匹配算法为零硬件依赖的纯逻辑（`main/place_fp.c`），主机单元测试 `tests/test_place_fp.c` 覆盖完全相同／部分重叠／完全不同／恰好 50% 边界四种情况，并接入 `tools/validate.sh` 静态门禁。最多保存 16 个地点（每地点 8 个 BSSID＋名称＋到访次数，整体为约 1 KB 的单个 NVS blob，namespace `placemem`），断电保留；新地点立即落盘，到访次数 5 分钟节流落盘、退出页面补刷一次。单页面显示「已知地点 #N／新地点／未知地点」状态、指纹强度（平均 RSSI：STRONG／GOOD／WEAK／POOR）与已保存地点数；UP/DOWN 循环浏览已保存地点，长按 OK 返回菜单。阻塞的扫描与 NVS 操作全部在常驻、队列驱动的 worker 任务中执行，地点库由互斥量保护，worker 更新 UI 时短持 LVGL 锁且页面删除后自动空转。
- 菜单网格为 10 个玩法重排：卡片改为高 38px、行距 45px，吉祥物下移至草地站位，消除第五行卡片与吉祥物的重叠。
- 修复 `tools/install-actionlint.sh` 在 macOS 上的校验失败：系统自带 BSD `sha256sum` 不支持 GNU `--check/--status`，Darwin 下改用 `shasum` 校验，Linux 仍用 GNU `sha256sum`。
- 新增 `Answer` 玩法（「答案之书」）：心中默念问题后按 OK 翻书，经过 1.3 秒悬念动画（「?」闪烁、省略号进度、合成翻书 whoosh 音效）后，随机中文答案淡入并伴随三音揭晓铃声；UP/DOWN 切换三副牌组（经典／打工人／程序员，每副 16 条，数据源为 `main/answers.json`），翻书动画期间切牌组防抖忽略，连续两次抽题不会重复。中文渲染使用 24px bpp4 子集字体（`main/answer_24.c`，176 个字形），由 `tools/gen_answer_data.js` 配合 `lv_font_conv` 生成，字库驻留 Flash、零 RAM 占用；PCM 音效全部运行时合成，由常驻、队列驱动的 worker 任务播放，全程零 NVS 写入。
- 新增 `WoodFish` 玩法（「电子木鱼」）：OK 短按敲击积累功德，连击越高暴击（+10）概率越高；UP 切换三款皮肤（经典木色／法喜金／电子紫），DOWN 切换自动敲击节奏（关／慢／中／快）；功德经批量 NVS 落盘（每 20 击或每 5 秒，退出时补刷一次）；阻塞的音频／Flash／I2C 操作全部在常驻队列 worker 任务中执行，LVGL 视觉反馈用零内存分配的底色闪烁替代逐帧 zoom 缩放动画。
- 修复 `Answer` 玩法真机白屏：中文子集字体由 lv_font_conv 默认压缩位图格式生成，而固件未开启 `CONFIG_LV_USE_FONT_COMPRESSED`，LVGL 对每个汉字静默返回空位图（书名、牌组名、答案全部不可见）；`main/answer_24.c` 已改用 `--no-compress` 重新生成。同时修复翻书页阴影残留：`ui_pixel_panel_create()` 把阴影挂在屏幕而非面板上，书页隐藏后阴影仍留在屏幕上盖住合着的封面；书页改为自建透明容器、阴影作为子对象，隐藏容器即整体消失。
